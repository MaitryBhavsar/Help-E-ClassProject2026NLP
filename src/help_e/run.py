...
# Additional retrieval baselines (additive, do not affect v1/v3/v4/v6):
#   rag      — BM25 over multi-session transcript, no graph, 1 LLM call/turn
#   graphrag — incremental free-text entity graph, 2 LLM calls/turn
SYSTEMS: tuple[str, ...] = (
    "v1", "v3", "v4", "v6", "v7", "v8", "rag", "graphrag", "cami",
)


def _pick_turn_fn_v6_aligned(system: str):
    """v6-aligned dispatcher. ALL systems (v1, v3, v4, v6, rag, graphrag)
    now run through the v6 driver (`run_profile_v6`) with a per-system
    `turn_fn`. Guarantees the ablation contract: shared simulator,
    shared MISC vocabulary, shared TTM enum, shared 3-field response
    output. v1/v3/v4/rag/graphrag share `response_simple`; v6 uses
    `response_v6` (HBM-aware).
    """
    if system == "v1":
    ...
    if system == "v6":
        ...
        return v6_turn_fn
    if system == "rag":
        from .baselines.rag_baseline import rag_turn_fn
        return rag_turn_fn
    if system == "graphrag":
        from .baselines.graphrag_baseline import graphrag_turn_fn
        return graphrag_turn_fn
    if system == "v7":
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--turns-list", type=str, default=None,
        help=(
            "Per-session turn counts as a comma-separated list "
            "(e.g., '20,10,10'). When set, overrides --sessions and "
            "--turns. Used by v7's 3-session 20/10/10 layout."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.turns_list:
        try:
            turns_list = [int(x.strip()) for x in args.turns_list.split(",") if x.strip()]
        except ValueError as e:
            raise SystemExit(f"--turns-list must be comma-separated ints: {e}")
        if not turns_list or any(t <= 0 for t in turns_list):
            raise SystemExit("--turns-list must contain positive integers")
    run_cfg = RunConfig(
        sessions_per_profile=(
            len(turns_list) if turns_list is not None else args.sessions
        ),
        turns_per_session=args.turns,
        turns_per_session_list=turns_list,
    ...
    if turns_list is not None:
        log.info(
            "starting run: system=%s profiles=%d sessions=%d turns_list=%s parallel=%d",
            args.system, len(profiles), len(turns_list), turns_list, parallelism,
        )
    else:
        log.info(
